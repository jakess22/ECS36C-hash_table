...
public:
    ...
    unsigned numElements() const;
		...
		unsigned getKey(unsigned slot) const;
		ValueType getValue(unsigned slot) const;
		bool isFilled(unsigned slot) const;
		bool wasDeleted(unsigned slot) const;
    ...
    const ValueType* get(unsigned key) const;
		...
		// returns address of pair the key belongs to
		Pair<ValueType>* getPair(unsigned cmp_key) const;
    ...
    /**
     *********** TODO: Returns a newly constructed hash table that is the hash table ***********
...
private:
    // TODO: Your members here.
		unsigned size; // may need to be size_t
		std::unique_ptr<Pair<ValueType>[]> table;
		unsigned lambda;
};
